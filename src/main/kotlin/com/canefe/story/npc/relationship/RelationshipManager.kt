...
class RelationshipManager(
    ...
    private fun isNameMentioned(
        ...
        for (part in nameParts) {
            ...
            if (part.lowercase() in
                listOf("elder", "lord", "lady", "sir", "master", "miss", "mrs", "mr", "dr", "the")
            ) {
                continue
            }
     ...
     */
    private fun findMatchingNames(
        aiTargets: List<String>,
    ...
    fun generateRelationshipLabel(
        ...
        val memorySummary =
            memory?.let {
                "Recent memory: ${it.content} (strength: ${String.format(
                    "%.2f",
                    it.getCurrentStrength(plugin.timeService),
                )})"
            } ?: "No recent memories."
