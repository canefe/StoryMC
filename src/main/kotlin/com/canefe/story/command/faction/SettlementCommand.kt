        ...
        CommandAPICommand("settlement")
            ...
            .executes(
                CommandExecutor { sender, args ->
                    sender.sendMessage(
                        "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlement Commands ${ChatColor.GOLD}===",
                    )
        ...
        return CommandAPICommand("create")
            ...
            .executes(
                CommandExecutor { sender, args ->
                    ...
                    sender.sendMessage(
                        "${ChatColor.GREEN}Created settlement ${settlement.name} in faction ${faction.name}",
                    )
            ...
            .executes(
                CommandExecutor { sender, args ->
                    ...
                    if (faction.removeSettlement(settlementId)) {
                        sender.sendMessage(
                            "${ChatColor.GREEN}Deleted settlement ${settlement.name} from faction ${faction.name}",
                        )
                        Story.instance.factionManager.saveAllFactions()
                    } else {
            ...
            .executes(
                CommandExecutor { sender, args ->
                    ...
                    if (factionId != null) {
                        ...
                        sender.sendMessage(
                            "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlements in ${faction.name} ${ChatColor.GOLD}===",
                        )
                        ...
                        sender.sendMessage(
                            "${ChatColor.GOLD}=== ${ChatColor.YELLOW}All Settlements ${ChatColor.GOLD}===",
                        )
                        var found = false
        ...
        return CommandAPICommand("mine")
                ...
                CommandAPICommand("listchests")
                    ...
                    .executes(
                        CommandExecutor { sender, args ->
                            ...
                            // Display chests
                            sender.sendMessage(
                                "${ChatColor.GOLD}=== ${ChatColor.YELLOW}${settlement.name} Mine Chests ${ChatColor.GOLD}===",
                            )
                    ...
                    .executes(
                        CommandExecutor { sender, args ->
                            ...
                            if (settlement.removeMineChest(chestIndex - 1)) { // Convert to 0-based index
                                sender.sendMessage(
                                    "${ChatColor.GREEN}Removed chest #$chestIndex from ${settlement.name}'s mine system.",
                                )
                                Story.instance.factionManager.saveAllFactions()
                            } else {
            ...
            .executes(
                CommandExecutor { sender, _ ->
                    sender.sendMessage(
                        "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlement Mine Commands ${ChatColor.GOLD}===",
                    )
    ...
    private fun createSettlementTreasuryCommand(): CommandAPICommand {
        return CommandAPICommand("treasury")
            ...
            .executes(
                CommandExecutor { sender, _ ->
                    sender.sendMessage(
                        "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlement Treasury Commands ${ChatColor.GOLD}===",
                    )
                    ...
                    sender.sendMessage(
                        "${ChatColor.GRAY}/settlement treasury update - Update physical currency display",
                    )
                },
            )
    }
    ...
    private fun createSettlementLeaderCommand(): CommandAPICommand {
        return CommandAPICommand("leader")
                ...
                CommandAPICommand("generate")
                    ...
                    .executes(
                        CommandExecutor { sender, args ->
                            ...
                            sender.sendMessage(
                                "${ChatColor.GRAY}Traits: ${leader.traits.joinToString(", ") { it.name }}",
                            )

                            // Record the event
                            settlement.addHistoryEntry(
                                "New Leader",
                                "${leader.title} ${leader.name} has become the new leader",
                            )
                ...
                CommandAPICommand("info")
                    ...
                    .executes(
                        CommandExecutor { sender, args ->
                            ...
                            // Display leader info
                            sender.sendMessage(
                                "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Leader of ${settlement.name} ${ChatColor.GOLD}===",
                            )
                            sender.sendMessage(
                                "${ChatColor.GRAY}Name: ${ChatColor.YELLOW}${leader.title} ${leader.name}",
                            )
                            ...
                            sender.sendMessage(
                                "${ChatColor.GRAY}- Stewardship: ${ChatColor.WHITE}${leader.stewardship}",
                            )
                            ...
                            sender.sendMessage(
                                "${ChatColor.GRAY}Traits: ${ChatColor.WHITE}${leader.traits.joinToString(
                                    ", ",
                                ) { it.name }}",
                            )
                        },
            ...
            .executes(
                CommandExecutor { sender, _ ->
                    sender.sendMessage(
                        "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlement Leader Commands ${ChatColor.GOLD}===",
                    )
    ...
    private fun createSettlementConfigCommand(): CommandAPICommand {
        return CommandAPICommand("config")
                ...
                CommandAPICommand("settaxrate")
                    ...
                    .executes(
                        CommandExecutor { sender, args ->
                            ...
                            sender.sendMessage(
                                "${ChatColor.GREEN}Set ${settlement.name}'s tax rate to ${(taxRate * 100).toInt()}%",
                            )
            ...
            .executes(
                CommandExecutor { sender, _ ->
                    sender.sendMessage(
                        "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlement Config Commands ${ChatColor.GOLD}===",
                    )
    ...
    private fun createSettlementCollectCommand(): CommandAPICommand {
        return CommandAPICommand("collect")
            ...
            .executes(
                CommandExecutor { sender, _ ->
                    sender.sendMessage(
                        "${ChatColor.GOLD}=== ${ChatColor.YELLOW}Settlement Collection Commands ${ChatColor.GOLD}===",
                    )
