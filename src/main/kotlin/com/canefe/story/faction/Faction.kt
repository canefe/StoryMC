...
data class Faction(
    ...
    fun handleRebellion(settlement: Settlement) {
        settlement.addAction(
            LeaderActionType.REBELLION,
            "The people of ${settlement.name} have rebelled against $name!",
        )
    ...
    fun getStatsDisplay(): List<String> {
        ...
        result.add(
            "${ChatColor.GRAY}Treasury: ${ChatColor.GOLD}${treasuryBalance.setScale(2, RoundingMode.HALF_UP)} coins",
        )
